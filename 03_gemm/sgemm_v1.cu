...
__global__ void sgemm_V1(
    /*
    参看知乎大佬讲解：https://zhuanlan.zhihu.com/p/703256080
    */
...
int main(void) {
    ...
    for (int i = 0; i < TESTNUM; i++) {
        ...
        dim3 blockDim(BN / TN, BM / TM); // 将这个大小为 BM*BN的block块儿 再次 切分为大小为 TM*TN 的更小的块儿，这种更小的块儿中包含TM*TN个元素， 每个线程处理这些个小块儿，也就是说，每个线程处理TM*TN个元素
        dim3 gridDim((N + BN - 1) / BN, (M + BM - 1) / BM);  // 将矩阵C分解成为 大小为 BM * BN 的block分块儿, 每个 BM*BN的块儿由一个block进行运算
